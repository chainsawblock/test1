...
type Props = { password: string; userInputs?: string[] };
...
export default function PasswordStrength({ password, userInputs = [] }: Props) {
  ...
  const bars = 5;
  const activeBars = Math.min(bars, Math.max(1, score + 1));
  const colorByScore =
    score <= 1 ? "bg-red-500" :
    ...
    <div className="mt-3 space-y-2">
      <div className="flex gap-1" aria-label="Индикатор сложности пароля">
        {Array.from({ length: bars }).map((_, i) => (
          <div key={i} className={`h-1.5 flex-1 rounded ${i < activeBars ? colorByScore : "bg-zinc-800"}`} />
        ))}
      </div>
